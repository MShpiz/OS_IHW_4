...
struct sockaddr_in logServAddres;

char echoBuffer[RCVBUFSIZE]; /* Buffer for echo string */
int recvMsgSize;             /* Size of received message */


void sendQueueRes(int pid) {
  char str[100];
  sprintf(str, "Client %d came in and waits in line", pid);
  int len = strlen(str);
  //printf("log_soket %d\n", logSocket);
  //printf("%s %d", str, len);
      if (sendto(logSocket, str, strlen(str), 0, (struct sockaddr *) &logServAddres, sizeof(logServAddres)) != strlen(str))
        DieWithError("send() failed");
}

void sendClientOut(int res, int pid) {
  char str[100];
  if (res) {
...
void HandleTCPClient(int sock, int workTime, int q_len, int logSoc, struct sockaddr_in logAddr) {
  logSocket = logSoc;
  logServAddres = logAddr;
  ...
  while (1) /* zero indicates end of transmission */
  {
    ...
    fill_queue(sock);
      ...
      while (last > 0) {
        ...
        sendClientOut(1, atoi(echoBuffer));
        echoBuffer[l] = '|';
        echoBuffer[l+1] = '\0';
